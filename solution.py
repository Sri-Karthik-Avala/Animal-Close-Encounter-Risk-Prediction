...
FOLDS = int(os.environ.get("ERIS_FOLDS", "8"))
...
FREEZE = os.environ.get("ERIS_FREEZE", "1") == "1"
RADCH = int(os.environ.get("ERIS_RADCH", "0"))
...
_p0 = np.asarray(Image.open(ROOT / train_df.image_path.values[0]).convert("RGB"))
NATIVE = (_p0.shape[1] - 4) // 2
log("native panel size", NATIVE)


def load_split(df):
    n = len(df)
    A = np.zeros((n, NATIVE, NATIVE), dtype=np.uint8)
    B = np.zeros((n, NATIVE, NATIVE), dtype=np.uint8)
...
def macro_score(yt, p, reg):
    minn = max(50, len(yt) // 25)
    vals = []
    for r in np.unique(reg):
        m = reg == r
        if m.sum() >= minn and 0 < yt[m].sum() < m.sum():
...
def set_res(r):
    ...
    c = (RES - 1) / 2.0
    rad = np.sqrt((xx - c) ** 2 + (yy - c) ** 2) / (RES / 2.0)
    RAD_T = torch.from_numpy((rad - 0.7).astype(np.float32))[None, None]
    for _s in globals().get("SPECS", []):
        if not _s["polar"]:
            _s["rad"] = RAD_T
...
DUAL = os.environ.get("ERIS_DUAL", "0") == "1"
...
def make_batch(A, B, LO, SC, idx, aug, gen=None, spec=None):
    ...
    if spec["polar"]:
        ...
        if RADCH:
            p = torch.cat([p, spec["rad"].expand(p.shape[0], 1, spec["pr"], spec["pa"])], dim=1)
        return p
    n = x.shape[0]
    if aug:
        ang = torch.rand(n, generator=gen) * 2 * math.pi
    ...
    theta[:, 1, 0] = sin * flip
    theta[:, 1, 1] = cos
    grid = F.affine_grid(theta, (n, x.shape[1], RES, RES), align_corners=False)
    x = F.grid_sample(x, grid, mode="bilinear", padding_mode="zeros", align_corners=False)
    if RADCH:
        x = torch.cat([x, RAD_T.expand(n, 1, RES, RES)], dim=1)
    return x
...
class Net(nn.Module):
    def __init__(self, spec, arch=ARCH):
        ...
        if 3 + RADCH != 3:
            ...
            c1 = nn.Conv2d(3 + RADCH, 64, 7, 2, 3, bias=False)
            c1.weight.data[:, :3] = w
...
RES_CANDS = [int(v) for v in os.environ.get("ERIS_RESCANDS", "224,208,192,176,160").split(",")]


def pick_res():
    if len(SPECS) != 1 or SPECS[0]["polar"]:
        return RES
    spec = SPECS[0]
    spe = max(1, math.ceil((len(train_df) * (FOLDS - 1) / FOLDS) / BS))
    idx = np.arange(min(BS, len(train_df)))
    yb = torch.from_numpy(y[idx]).to(DEV)
    base = RES_CANDS[-1]
    set_res(base)
    m = Net(spec).to(DEV)
    opt = torch.optim.AdamW([p for p in m.parameters() if p.requires_grad], lr=LR)
    xb = make_batch(trA, trB, trLO, trSC, idx, aug=True, spec=spec).to(DEV)
    for _ in range(2):
        opt.zero_grad(); F.binary_cross_entropy_with_logits(m(xb), yb).backward(); opt.step()
    t0 = time.time()
    for _ in range(5):
        opt.zero_grad(); F.binary_cross_entropy_with_logits(m(xb), yb).backward(); opt.step()
    tstep = (time.time() - t0) / 5.0
    m.eval()
    with torch.no_grad():
        t1 = time.time()
        for _ in range(max(1, NTTA)):
            m(xb)
        tinf = (time.time() - t1) / len(idx) * (len(test_df) + len(train_df) / FOLDS)
    del m, opt, xb
    chosen = base
    for r in RES_CANDS:
        k = (r / float(base)) ** 2.2
        total = FOLDS * (EPOCHS * tstep * spe * k + tinf * k)
        need = (time.time() - T0) + total + 0.06 * DEADLINE
        log(f"  res {r}: epoch~{tstep * spe * k:.0f}s total~{total:.0f}s need~{need:.0f}/{DEADLINE:.0f}")
        if need <= DEADLINE:
            chosen = r
            break
    set_res(chosen)
    log(f"selected RES={chosen}")
    return chosen


if os.environ.get("ERIS_ADAPTRES", "1") == "1":
    pick_res()
