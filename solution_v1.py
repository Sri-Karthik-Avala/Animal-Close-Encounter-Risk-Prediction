...
def make_batch(A, B, LO, SC, idx, aug, gen=None):
...
class Net(nn.Module):
    def __init__(self, arch=ARCH):
        ...
        self.bn = nn.BatchNorm1d(32 * nr2 + 64 * nr3)
        self.drop = nn.Dropout(0.3)
        self.head = nn.Linear(nf + 32 * nr2 + 64 * nr3, 1)
...
@torch.no_grad()
def predict(model, A, B, LO, SC, ntta=NTTA, bs=64):
    model.eval()
    n = len(A)
    ...
    for t in range(max(1, ntta)):
        for s in range(0, n, bs):
            ...
            xb = make_batch(A, B, LO, SC, idx, aug=False)
            if t:
                xb = torch.rot90(xb, t, dims=(2, 3))
...
def train_fold(f, fold_end):
    ...
    for ep in range(EPOCHS):
        est = float(np.median(EP_T)) if EP_T else 0.0
        if ep > 0 and (time.time() - T0) + est > fold_end:
            log(f"fold {f}: budget stop at epoch {ep} (est epoch {est:.0f}s)")
            break
        ep_t0 = time.time()
        model.train()
        ...
        for s in range(0, len(perm), BS):
            idx = perm[s:s + BS]
            xb = make_batch(trA, trB, trLO, trSC, idx, aug=True, gen=gen).to(DEV)
        ...
        EP_T.append(time.time() - ep_t0)
        log(f"fold {f} ep {ep} loss {tot / len(perm):.4f} ({EP_T[-1]:.0f}s)")
    vp = predict(model, trA[vai], trB[vai], trLO[vai], trSC[vai])
    log(f"fold {f} val util {regime_utility(y[vai], vp):.4f} ap {average_precision(y[vai], vp):.4f}")
    return model, vai, vp
...
_probe = Net().to(DEV)
_p0 = time.time()
predict(_probe, teA[:NP], teB[:NP], teLO[:NP], teSC[:NP])
T_TEST_EST = (time.time() - _p0) * (len(test_df) / NP) * 1.25
del _probe
log(f"inference probe: est {T_TEST_EST:.0f}s per model (test+val)")

T_TEST = []
for f in range(FOLDS):
    left = FOLDS - f
    now = time.time() - T0
    reserve = (float(np.median(T_TEST)) * 1.25 if T_TEST else T_TEST_EST) * left + 40.0
    avail = DEADLINE - now - reserve
    if avail <= 0:
        if nmodel >= 1:
            log(f"stopping before fold {f}: no budget left (reserve {reserve:.0f}s)")
            break
        avail = max(150.0, DEADLINE - now - 90.0)
    fold_end = now + avail / left
    log(f"fold {f}: budget until {fold_end:.0f}s (reserve {reserve:.0f}s, {left} folds left)")
    model, vai, vp = train_fold(f, fold_end)
    oof[vai] = vp
    tt0 = time.time()
    test_acc += predict(model, teA, teB, teLO, teSC)
    T_TEST.append(time.time() - tt0)
    nmodel += 1
    write_submission(test_acc / nmodel)
    log(f"fold {f} done, submission updated ({nmodel} models)")
    del model

test_p = test_acc / max(1, nmodel)
have = ~np.isnan(oof)
log(f"OOF covered {have.sum()}/{len(oof)} from {nmodel} models")
